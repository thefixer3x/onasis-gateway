 ...
 * Full implementation with all banking operations (JavaScript version)
 */

const XpressWalletService = require('./xpress-wallet-service.js');
const { Pool } = require('pg');

class XpressWalletWaaSAdapter {
  constructor(config = {}) {
    this.id = config.id || 'xpress-wallet-waas';
    this.name = 'Xpress Wallet WaaS';
    this.version = '2.0.0';
    this.description = 'Enhanced Xpress Wallet adapter with full Wallet-as-a-Service features';
  ...
  toolRequiresDb(toolName) {
    // Many tools in this adapter are DB-backed (wallet.* tables). If DATABASE_URL
    // isn't configured/reachable, fail fast with a clear error instead of
    // throwing "Cannot read properties of undefined" later.
    const dbTools = new Set([
      'get-wallet-details',
      'freeze-wallet',
      'unfreeze-wallet',
      'get-bank-list',
  ...
  async initialize() {
    ...
    this.service = new XpressWalletService({
      apiKey: this.config.apiKey || process.env.XPRESS_API_KEY || '',
      secretKey: this.config.secretKey || process.env.XPRESS_SECRET_KEY || '',
      merchantId: this.config.merchantId || process.env.XPRESS_MERCHANT_ID || '',
      baseUrl: this.config.baseUrl || 'https://api.xpress-wallet.com/v1',
      webhookSecret: this.config.webhookSecret || process.env.XPRESS_WEBHOOK_SECRET || '',
      environment: (this.config.environment || 'production')
    }, this.db);

    // Test database connection - handle gracefully if not available
    try {
      ...
      this.dbAvailable = false;
      console.warn(
        '⚠️ Xpress Wallet database connection failed. DB-backed tools will be unavailable until DATABASE_URL is configured/reachable:',
        error.message
      );
      this._initialized = true;
    }
  ...
  async executeTool(name, args, context) {
    if (this.toolRequiresDb(name) && !this.dbAvailable) {
      const err = new Error(
        `Tool "${name}" requires database connectivity. Set DATABASE_URL (and ensure the DB is reachable) to use DB-backed Xpress Wallet tools.`
      );
  ...
  async healthCheck() {
    try {
      ...
      if (this.dbAvailable) {
        ...
        // Best-effort API check using an existing wallet id.
        const testWallet = await this.db.query(
          'SELECT external_wallet_id FROM wallet.customer_wallets LIMIT 1'
        );
        if (testWallet.rows[0]) {
          // Prefer the current service method name; alias exists for backwards-compat.
          if (typeof this.service.getWalletBalance === 'function') {
            await this.service.getWalletBalance(testWallet.rows[0].external_wallet_id);
          } else if (typeof this.service.getXpressWalletBalance === 'function') {
            await this.service.getXpressWalletBalance(testWallet.rows[0].external_wallet_id);
          }
          result.api = true;
        }
      } else {
        // No DB: we can't query for a wallet id; do a shallow API-only check if available.
        if (typeof this.service.listBanks === 'function') {
          await this.service.listBanks('NG');
          result.api = true;
        }
      }

      result.healthy = result.db ? true : result.api;
      return result;
    } catch (error) {
      console.error('Health check failed:', error);
      return { healthy: false, db: this.dbAvailable, api: false, error: error.message };
    }
  }
  ...
  async getStats() {
    ...
    // Get additional metrics (DB-backed)
    let totalWallets = null;
    let dailyTransactions = null;
    if (this.dbAvailable) {
      const [walletCount, txCount] = await Promise.all([
        this.db.query('SELECT COUNT(*) FROM wallet.customer_wallets'),
        this.db.query('SELECT COUNT(*) FROM wallet.transactions WHERE created_at > NOW() - INTERVAL \'24 hours\''),
      ]);
      totalWallets = parseInt(walletCount.rows[0].count, 10);
      dailyTransactions = parseInt(txCount.rows[0].count, 10);
    }
...
module.exports = XpressWalletWaaSAdapter;
